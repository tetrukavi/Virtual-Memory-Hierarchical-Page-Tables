...
TEST(FlowTests, FlowTest)
{
    ...
    // Pushing the value 1337 into virtual page 3 in "disk" so when restore is called it'll be brought into memory.
    swapFile[3] = page_t{1337, 0};

    ASSERT_EQ(VMread(6, &gotten), 1) << "VMread(6, &gotten) should succeed";
    ASSERT_EQ(gotten, 1337) << "VMread(6, &gotten) should've yielded 1337, because of swapFile[3] = page_t{1337, 0};";
    // see pdf pages 15-16
    expectedAddrToValMap = {
        ...
        {14, 1337},
        {15, 0}
        // should have been restored during read since
        // virtual address 6 should map to physical address 14.
    ...
    ASSERT_EQ(valueAtPhysAddr15, 0) << "Nothing in thee've touched physical address 15";
    // the virtual address 31 which is in page 15 will map to physical address 15 (see PDF why this is true)
    swapFile[15] = {1337, 7331};
    ...
    expectedAddrToValMap = {
            ...
            {12, 0},
            {13, 0},
            // should have been restored from disk as page 15.
            {14, 1337},
            {15, 7331}
    };
